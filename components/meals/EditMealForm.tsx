      ...
      if (imageFiles.length > 0) {
        ...
        try {
          ...
          for (let i = 0; i < imageFiles.length && i < 2; i++) {
            ...
            uploadFormData.append('property_id', meal.id)
          ...
          if (uploadedUrls.length > 0) {
            toast.success(`식사와 이미지 ${uploadedUrls.length}개가 수정되었습니다`)
          } else {
            toast.success('식사가 수정되었습니다')
          }
        } catch (error) {
          console.error('이미지 업로드 중 오류:', error)
          toast.dismiss()
          toast.success('식사는 수정되었으나 이미지 업로드 중 오류가 발생했습니다')
        }
      } else {
        toast.success('식사가 성공적으로 수정되었습니다')
      }
  ...
  return (
    <>
      <form onSubmit={handleSubmit}>
        <div className="space-y-6">
                ...
                <div className="space-y-2">
                  ...
                  <div className="flex gap-2">
                    <Input
                      ...
                      defaultValue={meal.meal_datetime}
                      required
                      disabled={isPending}
            ...
            <CardContent className="space-y-4 pt-6">
              <div className="grid gap-4 md:grid-cols-2">
                <div className="space-y-2">
                  ...
                  <Input
                    ...
                    defaultValue={meal.total_calories || ''}
                    disabled={isPending}
                    className="h-11"
                  />
                  <p className="text-xs text-muted-foreground">
                ...
                <div className="space-y-2">
                  ...
                  <Input
                    ...
                    defaultValue={meal.serving_size || ''}
                    disabled={isPending}
                    className="h-11"
                  />
                </div>
              ...
              <div className="space-y-2">
                ...
                <Input
                  ...
                  defaultValue={meal.water_intake || ''}
                  disabled={isPending}
                  className="h-11"
                />
                <p className="text-xs text-muted-foreground">
          ...
          {(meal.photo_url_1 || meal.photo_url_2) && (
            <Card className="border-2">
              <CardHeader className="bg-gradient-to-r from-green-50 to-emerald-50 dark:from-green-950/20 dark:to-emerald-950/20">
                <CardTitle>기존 사진</CardTitle>
                <CardDescription>현재 등록된 사진입니다</CardDescription>
              </CardHeader>
              <CardContent className="pt-6">
          ...
          <Card className="border-2">
            <CardHeader className="bg-gradient-to-r from-green-50 to-emerald-50 dark:from-green-950/20 dark:to-emerald-950/20">
              <CardTitle>사진 변경</CardTitle>
              <CardDescription>새 사진을 업로드하면 기존 사진을 덮어씁니다 (선택사항)</CardDescription>
            </CardHeader>
            <CardContent className="pt-6">
              <MultiImageUpload
          ...
          <div className="flex justify-end gap-4 sticky bottom-4 bg-background/80 backdrop-blur-sm p-4 rounded-lg border-2 shadow-lg">
            <Button type="button" variant="outline" asChild disabled={isPending} size="lg">
              <Link href={`/meals/${meal.id}`}>취소</Link>
            </Button>
      ...
      <AlertDialog open={showConfirmDialog} onOpenChange={setShowConfirmDialog}>
        <AlertDialogContent>
          <AlertDialogHeader>
            <AlertDialogTitle>식사 기록을 수정하시겠습니까?</AlertDialogTitle>
            <AlertDialogDescription>
              수정한 정보가 저장됩니다.
            </AlertDialogDescription>
          </AlertDialogHeader>
