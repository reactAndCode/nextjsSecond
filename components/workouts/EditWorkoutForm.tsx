      ...
      if (imageFiles.length > 0) {
        ...
        try {
          ...
          for (let i = 0; i < imageFiles.length && i < 2; i++) {
            ...
            uploadFormData.append('property_id', workout.id)
          ...
          if (uploadedUrls.length > 0) {
            toast.success(`운동과 이미지 ${uploadedUrls.length}개가 수정되었습니다`)
          } else {
            toast.success('운동이 수정되었습니다 (이미지 업로드 실패)')
          }
        } catch (error) {
          console.error('이미지 업로드 중 오류:', error)
          toast.dismiss()
          toast.success('운동은 수정되었으나 이미지 업로드 중 오류가 발생했습니다')
        }
      } else {
        toast.success('운동이 성공적으로 수정되었습니다')
      }
  ...
  return (
    <>
        ...
        <div className="space-y-6">
            ...
            <CardContent className="space-y-4 pt-6">
              <div className="grid gap-4 md:grid-cols-2">
                <div className="space-y-2">
                  ...
                  <Input
                    ...
                    defaultValue={workout.exercise_name}
                    required
                    disabled={isPending}
                    className="h-11"
                  />
              ...
              <div className="space-y-2">
                ...
                <Input
                  ...
                  defaultValue={workout.body_part || ''}
                  disabled={isPending}
                  className="h-11"
                />
                <p className="text-xs text-muted-foreground">
              ...
              <div className="grid gap-4 md:grid-cols-2">
                ...
                <div className="space-y-2">
                  ...
                  <Input
                    ...
                    defaultValue={workout.next_goal || ''}
                    disabled={isPending}
                    className="h-11"
                  />
                  <p className="text-xs text-muted-foreground">
          ...
          <div className="flex justify-end gap-4 sticky bottom-4 bg-background/80 backdrop-blur-sm p-4 rounded-lg border-2 shadow-lg">
            <Button type="button" variant="outline" asChild disabled={isPending} size="lg">
              <Link href={`/workouts/${workout.id}`}>취소</Link>
            </Button>
      ...
      <AlertDialog open={showConfirmDialog} onOpenChange={setShowConfirmDialog}>
        <AlertDialogContent>
          <AlertDialogHeader>
            <AlertDialogTitle>운동 기록을 수정하시겠습니까?</AlertDialogTitle>
            <AlertDialogDescription>
              수정한 정보가 저장됩니다.
            </AlertDialogDescription>
          </AlertDialogHeader>
